...
from .models import CustomUser, CohortQuiz
from .forms import ChallengeTestForm, LoginForm, RegisterForm, ForgotPasswordForm, ResetPasswordForm, EditUserForm, CohortQuizForm
...
def current_student_home(request):
    # Render the homepage for current students
    return render(request, "home/current_student_home.html")


@login_required
def future_student_home(request):
    # Render the homepage for future students
    return render(request, "home/future_student_home.html")


@login_required
def alumni_home(request):
    # Render the homepage for alumni
    return render(request, "home/alumni_home.html")

# Challenge Test View
def challenge_test(request, cohort):
    # Instantiate the ChallengeTestForm with the given cohort and POST data if available
